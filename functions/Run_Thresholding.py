...
This function runs thresholding on innovations, according to the
 ...
 Input:
   param - dict containing all necessary parameters to run TA
       ...
       ['force_Thresholding'] - if set to 1, thresholding will be forced
                          to run, even if already has been done
       'alpha'          - alpha-level at which to look for significance of
                          innovation signal frames (first element is the
                          percentile of the lower threshold - negative
                          innovations - and second element the upper
                          threshold one - positive innovations)
       'f_voxels'       - fraction of voxels from the ones entering total
                          activation for a given subject that should show
                          an innovation at the same time point, so that
                          the corresponding frame is retained for iCAPs
                          clustering
       'thresh_title'   - title used to create the folder where
                          thresholding data will be saved
       'threshold_minclussize'        - number of neighbours that must
                          also show an innovation for a voxel to be
                          retained
       'threshold_interconnectivity'  - number of neighbours to consider
                          in the process

 Output:
   Cycles through the subjects and thresholds their data:
       1) Temporal threshold: takes the innovation signals generated from
          the surrogate data, builds a distribution from them, samples
          the X-th percentiles from the distribution at each voxel and
          masks temporal frames from the real data with this
       2) Spatial threshold: marks frames where X percent of significant
          voxels show an innovation at the same time point
"""
...
def run_thresholding(param):
    ...
    for i_s in range(param["n_subjects"]):
        ...
        # path towards the subject we are interested in
        ...
        # Reset param to original so start of loop is fresh
        del param
        param = copy.deepcopy(param_CI)
